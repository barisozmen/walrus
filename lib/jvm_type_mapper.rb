...
module JVMTypeMapper
  ...
  def self.to_jvm(walrus_type)
    raise "to_jvm called with nil type" if walrus_type.nil?
    raise "to_jvm called with empty type" if walrus_type.to_s.empty?
    ...
    else raise "Unknown Walrus type: #{walrus_type.inspect}"
    end
  end
